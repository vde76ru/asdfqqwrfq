...
class UnifiedConfig:
    ...
    def __getattr__(self, name: str):
        ...
        """
        # Специальная обработка для параметров Whale Hunting
        whale_params = {
            'WHALE_MIN_USD_VALUE': 100000.0,
            'WHALE_EXCHANGE_FLOW_THRESHOLD': 500000.0,
            'WHALE_LOOKBACK_HOURS': 24,
            'WHALE_SIGNAL_CONFIDENCE': 0.7,
        }
        
        if name in whale_params:
            return whale_params[name]
        ...
        if name in producer_params:
            return producer_params[name]
