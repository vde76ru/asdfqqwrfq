...
class SignalAggregator:
    ...
    def _save_aggregated_signal(self, aggregated_data: Dict[str, Any], 
        ...
        try:
            ...
            if aggregated_data['confidence'] >= 0.7:
            try:
                from ..notifications.telegram import NotificationManager
                nm = NotificationManager()
                
                # Создаем фейковый объект сигнала для совместимости
                class SignalObj:
                    def __init__(self, data):
                        self.symbol = data['symbol']
                        self.action = data['final_signal_type']
                        self.strength = data['confidence']
                        self.price = 0  # Нужно получить из market data
                        self.stop_loss = None
                        self.take_profit = None
                        self.strategy = data['contributing_strategies']
                        self.indicators = data.get('metadata', {})
                
                signal_obj = SignalObj(aggregated_data)
                await nm.send_signal_notification(signal_obj)
                
            except Exception as e:
                logger.error(f"Ошибка отправки уведомления: {e}")
            if aggregated_data['confidence'] >= 0.7:
                asyncio.create_task(self._send_notification(aggregated_data, group))
    ...
    async def _send_notification(self, aggregated_data: Dict[str, Any], group: SignalGroup):
        """Отправка уведомления о новом агрегированном сигнале"""
        try:
            # Получаем NotificationManager
            from ..notifications.telegram import NotificationManager
            notification_manager = NotificationManager()
            
            # Находим лучший сигнал для деталей
            best_signal = max(group.signals, key=lambda s: s.confidence)
            
            # Подготавливаем рыночные данные
            market_data = {}
            if hasattr(best_signal, 'metadata') and best_signal.metadata:
                try:
                    metadata = json.loads(best_signal.metadata) if isinstance(best_signal.metadata, str) else best_signal.metadata
