...
class DataCollector:
    ...
    async def collect_historical_data(self, symbol: str, timeframe: str = '1h', limit: int = 200):
        """
    ...
    def _convert_klines_to_dataframe(self, klines_list: list, symbol: str, timeframe: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Bybit возвращает данные от новых к старым, переворачиваем для стандартного анализа
    ...
    def get_latest_candles(self, symbol: str, timeframe: str = '1h', limit: int = 100) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            with self.db_session() as session:
                ...
                interval = timeframe_mapping.get(timeframe, 60)  # Default to 1h
                ...
                data = []
                for candle in reversed(candles):  # Разворачиваем для хронологического порядка
                    data.append({
    ...
    async def get_latest_candles_async(self, symbol: str, timeframe: str = '1h', limit: int = 100) -> Optional[pd.DataFrame]:
        """
