...
class WhaleHuntingStrategy(BaseStrategy):
    ...
    async def analyze(self, symbol: str, df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        """
        Метод analyze для совместимости с BaseStrategy.
        Вызывает generate_signals для получения сигналов.
        
        Args:
            df: DataFrame с историческими данными (не используется в этой стратегии)
            symbol: Символ для анализа (не используется в этой стратегии)
            
        Returns:
            Dict с результатами анализа
        """
        # Вызываем основной метод генерации сигналов
        signals = await self.generate_signals()
        
        # Возвращаем результат в формате, ожидаемом базовым классом
        return {
            'signals': signals,
            'symbol': symbol,
            'strategy': self.name
    ...
    async def stop(self):
        """
        ✅ ИСПРАВЛЕНИЕ: Добавлен метод stop для корректного завершения работы.
        """
        logger.info(f"  Стратегия {self.name} остановлена.")
        # Здесь можно добавить логику очистки, если она потребуется
        pass
