    ...
    try:
        ...
        for symbol in bot_instance.active_pairs:
            try:
                ...
                active_strategies = getattr(bot_instance.config, 'ACTIVE_STRATEGIES', {
    ...
    try:
        ...
        if lengths and all(l == lengths[0] for l in lengths):
            ...
            if 'timestamp' in market_data:
                df['timestamp'] = pd.to_datetime(market_data['timestamp'], unit='ms')
...
def _prepare_market_data(bot_instance, symbol):
    """Подготовка рыночных данных для анализа"""
    try:
        # Получаем последние свечи из data_collector
        if hasattr(bot_instance, 'data_collector'):
            candles = bot_instance.data_collector.get_latest_candles(symbol, '1h', 100)
            if candles:
                return {
