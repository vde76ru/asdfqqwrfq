    ...
    try:
        ...
        if 'whale_hunting' in enabled_list:
            try:
                ...
                whale_config = {
                    'min_usd_value': float(getattr(config, 'WHALE_MIN_USD_VALUE', 100000)),
                    'exchange_flow_threshold': float(getattr(config, 'WHALE_EXCHANGE_FLOW_THRESHOLD', 500000))
                }
                ...
                logger.error(f"❌ Ошибка инициализации WhaleHunting: {e}")
        ...
        if 'sleeping_giants' in enabled_list:
            try:
                ...
                sg_config = {
                    ...
                    'volume_threshold': float(getattr(config, 'SLEEPING_GIANTS_VOLUME_THRESHOLD', 0.7))
                }
                ...
                logger.error(f"❌ Ошибка инициализации SleepingGiants: {e}")
        
        # Order Book Analysis
        if 'order_book_analysis' in enabled_list:
            try:
                from ...strategies.order_book_analysis import OrderBookAnalysisStrategy
                bot_manager.signal_strategies['order_book_analysis'] = OrderBookAnalysisStrategy('order_book_analysis')
                logger.info("✅ OrderBookAnalysisStrategy инициализирована")
            except Exception as e:
                logger.error(f"❌ Ошибка инициализации OrderBookAnalysis: {e}")
