...
# Импорты типов
from src.bot.internal.types import TradingOpportunity, MarketPhase, RiskLevel
...
async def _initialize_strategies(bot_manager):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Загружено {len(bot_manager.available_strategies)} стратегий")
        ...
        try:
            ...
            strategy_weights_config = getattr(bot_manager.config, 'STRATEGY_WEIGHTS', None)
            ...
            for strategy_name, weight in strategy_weights.items():
                if weight > 0 and strategy_name in bot_manager.available_strategies:
                    try:
                        ...
                        bot_manager.strategy_instances[strategy_name] = strategy_instance
                        bot_manager.strategy_performance[strategy_name] = {
                            'weight': weight,
                            'enabled': True,
                            'total_trades': 0,
                            'winning_trades': 0,
            ...
            if not bot_manager.strategy_instances:
                ...
                bot_manager.strategy_instances['basic'] = BasicStrategy()
                bot_manager.strategy_performance['basic'] = {
                    'weight': 100.0, 'enabled': True, 'total_trades': 0,
                    'winning_trades': 0, 'losing_trades': 0, 'total_profit': 0.0,
                    'win_rate': 0.0, 'last_used': None
                }

            logger.info(f"✅ Инициализировано {len(bot_manager.strategy_instances)} стратегий")
            active_strategies = [name for name, inst in bot_manager.strategy_instances.items()]
            logger.info(f"Активные стратегии: {', '.join(active_strategies)}")

            # Нормализуем веса
            total_weight = sum(
                perf['weight']
                for perf in bot_manager.strategy_performance.values()
                if perf.get('enabled', True)
            )
            if total_weight > 0:
                for strategy_name in bot_manager.strategy_performance:
                    if bot_manager.strategy_performance[strategy_name].get('enabled', True):
                        normalized_weight = (
                            bot_manager.strategy_performance[strategy_name]['weight'] / total_weight * 100
                        )
                        bot_manager.strategy_performance[strategy_name]['normalized_weight'] = normalized_weight
                        logger.debug(
                            f" {strategy_name}: вес {normalized_weight:.1f}% "
                            f"(оригинальный: {bot_manager.strategy_performance[strategy_name]['weight']})"
                        )
            return True
        except Exception as e:
