    ...
    while True:
        try:
            ...
            if hasattr(strategy, 'analyze'):
                # ✅ ИСПРАВЛЕНО: Передаем только symbol, как и ожидается
                if hasattr(bot_instance, 'active_pairs'):
                    for symbol in bot_instance.active_pairs:
                        try:
                            # Стратегия сама должна получать необходимые данные по символу.
                            signals = await strategy.analyze(symbol=symbol)
                            if signals:
                                # Обработка для стратегий, возвращающих словарь
                                ...
                                # Обработка для стратегий, возвращающих список
                            ...
                            logger.error(f"❌ {name}: ошибка анализа пары {symbol}: {pair_e}", exc_info=False) # exc_info=False чтобы не засорять лог
