...
class BybitDataProducer:
    ...
    async def _init_volume_history(self):
        ...
        # ✅ ИСПРАВЛЕНО: Определяем interval из конфигурации
        default_interval = getattr(config, 'DEFAULT_KLINE_INTERVAL', '5m')
        ...
        for symbol in self.symbols:
            try:
                ...
                klines = await self.exchange_client.get_klines(
                    symbol=symbol,
                    interval=default_interval,  # ✅ ИСПРАВЛЕНО: используем default_interval
                    limit=200
                )
