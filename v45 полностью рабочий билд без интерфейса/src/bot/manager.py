        ...
        for symbol in self.active_pairs:
            try:
                ...
                if self.order_book_analysis:
                    try:
                        # ✅ ИСПРАВЛЕНИЕ: Проверяем, что exchange_client инициализирован
                        exchange_client = self.exchange_client or getattr(self, 'enhanced_exchange_client', None)
                        if exchange_client:
                            signal = await self.order_book_analysis.analyze(symbol, exchange_client=exchange_client)
                            if signal:
                                strategies_data.append({
                            ...
                            logger.debug(f"exchange_client не инициализирован для order_book_analysis ({symbol})")
                    except Exception as e:
