    ...
    async def _get_market_data(self, symbol: str, timeframe: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if candles:
                ...
                data = []
                # Сортируем по возрастанию времени, так как из БД они приходят в обратном порядке
                for candle in reversed(candles):
                    data.append({
                        # ✅ ИСПРАВЛЕНО: Используем 'open_time' вместо несуществующего 'timestamp'
                ...
                if not data:
                    logger.warning(f"⚠️ Список свечей для {symbol} оказался пустым после обработки.")
                    return None

                df = pd.DataFrame(data)
                df.set_index('timestamp', inplace=True)
                
                logger.debug(f"📈 Получено {len(df)} свечей для {symbol} из БД.")
                return df
            else:
                logger.warning(f"⚠️ Нет данных в БД для {symbol} ({timeframe}).")
                return None
