...
class SleepingGiantsStrategy(BaseStrategy):
    ...
    def __init__(self,
        ...
        Args:
            volatility_threshold: Порог низкой волатильности
            volume_anomaly_threshold: Порог аномального объема
            hurst_threshold: Порог показателя Херста
            ofi_threshold: Порог Order Flow Imbalance
            min_confidence: Минимальная уверенность сигнала
        """
        # ✅ ИСПРАВЛЕНО: Правильная инициализация родительского класса
        # Вызываем конструктор родителя БЕЗ параметра name
        super().__init__()

        # Устанавливаем имя стратегии
        self.name = "sleeping_giants"
