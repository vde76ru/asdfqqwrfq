...
# =================================================================
# ФАБРИЧНАЯ ФУНКЦИЯ И ГЛОБАЛЬНЫЙ ДОСТУП
# =================================================================
...
def get_risk_manager() -> EnhancedRiskManager:
    """
    Фабричная функция для получения экземпляра Enhanced Risk Manager
    Реализует паттерн Singleton
    
    Returns:
        EnhancedRiskManager: Глобальный экземпляр менеджера рисков
    """
    global global_risk_manager
    
    if global_risk_manager is None:
        try:
            global_risk_manager = EnhancedRiskManager()
            logger.info("✅ Создан глобальный экземпляр EnhancedRiskManager")
        except Exception as e:
            logger.error(f"❌ Ошибка создания EnhancedRiskManager: {e}")
            raise
    
    return global_risk_manager
...
def create_risk_manager(risk_params: Optional[RiskParameters] = None) -> EnhancedRiskManager:
    ...
    Returns:
        EnhancedRiskManager: Новый экземпляр менеджера рисков
    """
...
def reset_risk_manager():
    ...
    global global_risk_manager
    global_risk_manager = None
    logger.info("🔄 Глобальный экземпляр EnhancedRiskManager сброшен")

def get_risk_manager_status() -> Dict[str, Any]:
    """
    Получение статуса системы управления рисками
    
    Returns:
        Dict[str, Any]: Подробный статус риск-менеджера
    """
    global global_risk_manager
    ...
    try:
        return {
            'initialized': True,
            'status': global_risk_manager.current_status.value,
            'emergency_stop': global_risk_manager.emergency_stop_active,
            'total_checks': global_risk_manager.total_risk_checks,
            'rejected_trades': global_risk_manager.rejected_trades,
            'risk_params': {
                'max_position_risk': global_risk_manager.risk_params.max_position_risk,
                'max_portfolio_risk': global_risk_manager.risk_params.max_portfolio_risk,
                'max_drawdown': global_risk_manager.risk_params.max_drawdown
            }
        }
    except Exception as e:
        return {
