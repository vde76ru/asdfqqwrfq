...
async def start_signal_system_loops(bot_instance):
    ...
    if bot_instance.onchain_producer:
        tasks.append(asyncio.create_task(
            bot_instance.onchain_producer.start_monitoring(),
            name="OnchainDataProducer"
        ))
        logger.info("▶️ Запущен OnchainDataProducer")
    
    if bot_instance.bybit_producer:
        tasks.append(asyncio.create_task(
            bot_instance.bybit_producer.start_monitoring(),
            name="BybitDataProducer"
        ))
        logger.info("▶️ Запущен BybitDataProducer")
    
    # Запускаем стратегии сигналов
    if hasattr(bot_instance, 'whale_hunting') and bot_instance.whale_hunting:
        tasks.append(asyncio.create_task(
            _run_signal_strategy(bot_instance, 'whale_hunting', 60),
            name="WhaleHuntingStrategy"
        ))
        logger.info("▶️ Запущена WhaleHuntingStrategy (интервал: 60с)")
    
    if hasattr(bot_instance, 'sleeping_giants') and bot_instance.sleeping_giants:
        tasks.append(asyncio.create_task(
            _run_signal_strategy(bot_instance, 'sleeping_giants', 300),
            name="SleepingGiantsStrategy"
        ))
        logger.info("▶️ Запущена SleepingGiantsStrategy (интервал: 300с)")
    
    if hasattr(bot_instance, 'order_book_analysis') and bot_instance.order_book_analysis:
        tasks.append(asyncio.create_task(
            _run_signal_strategy(bot_instance, 'order_book_analysis', 60),
            name="OrderBookAnalysis"
        ))
        logger.info("▶️ Запущена OrderBookAnalysis (интервал: 60с)")
    
    # ДОБАВЛЯЕМ ЦИКЛ ОБНОВЛЕНИЯ МАТРИЦЫ СИГНАЛОВ
    tasks.append(asyncio.create_task(
        _update_signals_matrix_loop(bot_instance),
        name="SignalsMatrixUpdate"
    ))
    logger.info("➕ Запущено обновление матрицы сигналов (интервал: 30c)")
    
    # Агрегатор сигналов
    if hasattr(bot_instance, 'signal_aggregator') and bot_instance.signal_aggregator:
        tasks.append(asyncio.create_task(
            _run_signal_aggregator(bot_instance),
            name="SignalAggregator"
        ))
        logger.info("▶️ Запущен SignalAggregator (интервал: 60с)")
    
    # Система уведомлений
    if hasattr(bot_instance, 'notification_manager') and bot_instance.notification_manager:
        tasks.append(asyncio.create_task(
            _run_notification_system(bot_instance),
            name="NotificationSystem"
        ))
        logger.info("▶️ Запущена система уведомлений")
    
    bot_instance.signal_tasks = tasks
    logger.info(f"✅ Запущено {len(tasks)} циклов анализа сигналов")
    return tasks
    
async def _update_signals_matrix_loop(bot_instance):
    ...
    while not bot_instance._stop_event.is_set():
        try:
            ...
            # Обновляем матрицу сигналов
            await bot_instance.update_signals_matrix()
            
            # Отправляем обновление через WebSocket
            if hasattr(bot_instance, 'socketio') and bot_instance.socketio:
                matrix_data = bot_instance.get_signals_matrix_data()
                bot_instance.socketio.emit('signals_matrix_update', {
                    'data': matrix_data,
            ...
            logger.error(f"❌ Ошибка в цикле обновления матрицы сигналов: {e}")
