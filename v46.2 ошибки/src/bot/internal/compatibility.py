...
def get_compatibility(bot_instance):
    ...
    class Compatibility:
        ...
        def stop(self) -> Tuple[bool, str]:
            """СИНХРОННАЯ обертка для остановки"""
            from .lifecycle import stop
            return stop(self.bot)
            
        def __repr__(self) -> str:
            """Строковое представление BotManager"""
            return __repr__(self.bot)
            
        def set_socketio(self, socketio_instance):
            """Установка SocketIO"""
            return set_socketio(self.bot, socketio_instance)
...
class Compatibility:
    ...
    def __repr__(bot_manager) -> str:
        """Строковое представление BotManager"""
        status = bot_manager.status.value if hasattr(bot_manager, 'status') else 'unknown'
        running = getattr(bot_manager, 'is_running', False)
        return f"<BotManager status={status} running={running}>"
    ...
    def set_socketio(bot_manager, socketio_instance):
        """Установка экземпляра SocketIO для WebSocket коммуникации"""
        bot_manager.socketio = socketio_instance
        logger.info("✅ SocketIO установлен для real-time обновлений")
