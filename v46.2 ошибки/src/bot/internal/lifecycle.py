...
def start(bot_manager) -> Tuple[bool, str]:
    ...
    # ✅ ИСПРАВЛЕНО: Используем threading.Event для межпоточного общения
    bot_manager._thread_stop_event = threading.Event()
    
    # Создаем поток, который будет управлять асинхронным циклом
    bot_manager._async_thread = threading.Thread(
        target=run_async_tasks, 
        args=(bot_manager,), 
        name="BotAsyncLoopThread"
    )
    bot_manager._async_thread.daemon = True 
    bot_manager._async_thread.start()
    
    # ✅ ИСПРАВЛЕНО: Ждем пока бот действительно запустится
    timeout = 10  # секунд
    ...
    while time.time() - start_time < timeout:
        if bot_manager.status == BotStatus.RUNNING:
            ...
            logger.info(msg)
            return True, msg
        elif bot_manager.status == BotStatus.ERROR:
            msg = f"Ошибка запуска бота. Проверьте логи."
            logger.error(msg)
            return False, msg
        time.sleep(0.5)
    
    # Таймаут
    msg = f"Таймаут запуска бота. Текущий статус: {bot_manager.status.value}"
    logger.error(msg)
    return False, msg
...
def stop(bot_manager) -> Tuple[bool, str]:
    ...
    # ✅ ИСПРАВЛЕНО: Используем thread event для сигнала остановки
    if hasattr(bot_manager, '_thread_stop_event'):
        bot_manager._thread_stop_event.set()
    
    # Также сигнализируем asyncio event если он существует
    if hasattr(bot_manager, '_stop_event') and bot_manager._stop_event:
        # Безопасно устанавливаем event в контексте loop
        if hasattr(bot_manager, '_async_thread') and bot_manager._async_thread.is_alive():
    ...
    if hasattr(bot_manager, '_async_thread'):
        bot_manager._async_thread.join(timeout=15)

        if bot_manager._async_thread.is_alive():
...
def run_async_tasks(bot_manager):
    ...
    try:
        ...
        # ✅ ИСПРАВЛЕНО: Создаем asyncio.Event в правильном потоке
...
async def start_async(bot_manager):
    """
    Основная асинхронная логика запуска бота.
    Выполняется в отдельном потоке с собственным event loop.
    """
    try:
        logger.info("🔄 Начало асинхронного запуска бота...")
        
        # ✅ ИСПРАВЛЕНО: Проверяем thread event для остановки
        def check_stop_signal():
            return (hasattr(bot_manager, '_thread_stop_event') and 
                   bot_manager._thread_stop_event.is_set())
        ...
        try:
            await load_pairs_from_config(bot_manager)
        except Exception as e:
        ...
        # ✅ ИСПРАВЛЕНО: Ожидание сигнала остановки с проверкой thread event
        while not check_stop_signal():
            if hasattr(bot_manager, '_stop_event'):
                try:
                    await asyncio.wait_for(bot_manager._stop_event.wait(), timeout=1.0)
                    break
