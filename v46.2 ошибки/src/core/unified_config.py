...
class UnifiedConfig:
    ...
    DEFAULT_KLINE_INTERVAL = os.getenv('DEFAULT_KLINE_INTERVAL', '5m')
    MAX_KLINES_LIMIT = int(os.getenv('MAX_KLINES_LIMIT', '100'))
    MAX_CONCURRENT_SYMBOLS = int(os.getenv('MAX_CONCURRENT_SYMBOLS', '5'))
    MAIN_CYCLE_INTERVAL = int(os.getenv('MAIN_CYCLE_INTERVAL', '30'))
    MULTI_INDICATOR_MIN_DATA = int(os.getenv('MULTI_INDICATOR_MIN_DATA', '20'))
    ...
    @classmethod
    def get_active_strategies(cls) -> Dict[str, float]:
        """
        Получение активных стратегий с их весами
        
        Returns:
            Dict[str, float]: Словарь стратегий с весами
        """
        # Сначала проверяем ACTIVE_STRATEGIES из окружения
        active_strategies_env = os.getenv('ACTIVE_STRATEGIES', '')
        
        if active_strategies_env:
            strategies = {}
            
            # Пробуем разные форматы
            # Формат 1: "strategy1:weight1,strategy2:weight2"
            if ':' in active_strategies_env:
                for item in active_strategies_env.split(','):
                    if ':' in item:
                        name, weight = item.split(':')
                        strategies[name.strip()] = float(weight.strip())
            # Формат 2: JSON строка
            elif active_strategies_env.startswith('{'):
                try:
                    import json
                    strategies = json.loads(active_strategies_env)
                except:
                    pass
            # Формат 3: Просто список стратегий через запятую
            else:
                for strategy in active_strategies_env.split(','):
        ...
        # Если ACTIVE_STRATEGIES не задан, используем STRATEGY_WEIGHTS
        if hasattr(cls, 'STRATEGY_WEIGHTS') and isinstance(cls.STRATEGY_WEIGHTS, dict):
            return cls.STRATEGY_WEIGHTS
        
        # Собираем из индивидуальных весов
        strategies = {}
        
        # Основные стратегии
        if cls.WEIGHT_MULTI_INDICATOR > 0:
            strategies['multi_indicator'] = cls.WEIGHT_MULTI_INDICATOR * 100
        if cls.WEIGHT_MOMENTUM > 0:
            strategies['momentum'] = cls.WEIGHT_MOMENTUM * 100
        if cls.WEIGHT_MEAN_REVERSION > 0:
            strategies['mean_reversion'] = cls.WEIGHT_MEAN_REVERSION * 100
        if cls.WEIGHT_BREAKOUT > 0:
            strategies['breakout'] = cls.WEIGHT_BREAKOUT * 100
        if cls.WEIGHT_SCALPING > 0:
            strategies['scalping'] = cls.WEIGHT_SCALPING * 100
        if cls.WEIGHT_SWING > 0:
            strategies['swing'] = cls.WEIGHT_SWING * 100
        
        # Дополнительные стратегии
        if hasattr(cls, 'WHALE_HUNTING_WEIGHT') and cls.WHALE_HUNTING_WEIGHT > 0:
            strategies['whale_hunting'] = cls.WHALE_HUNTING_WEIGHT
        if hasattr(cls, 'SLEEPING_GIANTS_WEIGHT') and cls.SLEEPING_GIANTS_WEIGHT > 0:
            strategies['sleeping_giants'] = cls.SLEEPING_GIANTS_WEIGHT
        if hasattr(cls, 'ORDER_BOOK_WEIGHT') and cls.ORDER_BOOK_WEIGHT > 0:
            strategies['order_book_analysis'] = cls.ORDER_BOOK_WEIGHT
        
        # Если ничего не нашли, возвращаем стандартный набор
        if not strategies:
            strategies = {
    ...
    def __getattr__(self, name: str):
        ...
        """
        # Специальная обработка для ACTIVE_STRATEGIES
        if name == 'ACTIVE_STRATEGIES':
