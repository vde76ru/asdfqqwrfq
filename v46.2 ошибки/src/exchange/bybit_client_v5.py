...
class BybitRateLimiter:
    """Менеджер rate limit для Bybit API"""
    ...
    async def wait_if_needed(self, endpoint_type: str = 'default'):
        ...
        try:
            ...
            window = self.request_windows[endpoint_type]
            limit = self.limits.get(endpoint_type, 10)
            ...
            if len(window) >= limit:
                sleep_time = 1.0 - (current_time - window[0]) + 0.1
                if sleep_time > 0:
...
_rate_limiter = BybitRateLimiter()

def rate_limited(endpoint_type: str = 'default'):
    """Декоратор для rate limiting"""
    def decorator(func):
        async def wrapper(*args, **kwargs):
            await _rate_limiter.wait_if_needed(endpoint_type)
...
class BybitClientV5:
    ...
    async def get_klines(self, category: str, symbol: str, 
        ...
        """Получение исторических данных (свечей) - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        try:
            # Преобразование интервалов для Bybit API v5
            interval_map = {
                '1m': '1', '3m': '3', '5m': '5', '15m': '15', '30m': '30',
                '1h': '60', '2h': '120', '4h': '240', '6h': '360', '12h': '720',
                '1d': 'D', '1w': 'W', '1M': 'M',
                # Прямые значения (если уже в формате Bybit)
                '1': '1', '3': '3', '5': '5', '15': '15', '30': '30',
                '60': '60', '120': '120', '240': '240', '360': '360', '720': '720',
                'D': 'D', 'W': 'W', 'M': 'M'
            }
            
            # Получаем правильный интервал
            api_interval = interval_map.get(interval, interval)
            
            # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Добавляем временные метки
            # Bybit требует временные метки в миллисекундах
            end_time = int(time.time() * 1000)  # Текущее время
            
            # Вычисляем start_time в зависимости от интервала
            interval_minutes = {
                '1': 1, '3': 3, '5': 5, '15': 15, '30': 30,
                '60': 60, '120': 120, '240': 240, '360': 360, '720': 720,
                'D': 1440, 'W': 10080, 'M': 43200  # Примерные значения
            }
            
            minutes = interval_minutes.get(api_interval, 60)
            # Добавляем запас в 10% для гарантии получения нужного количества свечей
            start_time = end_time - (limit * minutes * 60 * 1000 * 1.1)
            ...
            params = {
                ...
                "limit": limit,
                "start": int(start_time),  # ВАЖНО: start в миллисекундах
                "end": int(end_time)       # ВАЖНО: end в миллисекундах
            }
            
            logger.info(f"🔍 Запрос klines с параметрами: {params}")
            ...
            if response.get('retCode') == 0:
                ...
                if not klines_list:
                    logger.warning(f"⚠️ Пустой список свечей для {symbol} {interval}")
                    # Попробуем альтернативный запрос без временных меток
                    params_alt = {
                        "category": category,
                        "symbol": symbol,
                        "interval": api_interval,
                    ...
                    logger.info(f"🔄 Повторный запрос без временных меток: {params_alt}")
                    response = await self._make_request('GET', '/v5/market/kline', params_alt)
                    
                    if response.get('retCode') == 0:
                        result = response.get('result', {})
                        klines_list = result.get('list', [])
                        logger.info(f"📊 Получено свечей (альтернативный запрос): {len(klines_list)}")
                else:
                    logger.info(f"📊 Получено свечей: {len(klines_list)}")
                    
                    # Проверяем формат данных
                    if klines_list and isinstance(klines_list[0], list):
                        logger.debug(f"✅ Формат свечей корректный. Пример: {klines_list[0]}")
                    else:
                        logger.warning(f"⚠️ Неожиданный формат свечей: {type(klines_list[0]) if klines_list else 'empty'}")
            else:
                logger.error(f"❌ Ошибка API: retCode={response.get('retCode')}, retMsg={response.get('retMsg')}")
                
                # Специальная обработка для некоторых ошибок
                if response.get('retCode') == 10001:  # Parameter error
                    logger.error(f"❌ Ошибка параметров. Проверьте символ {symbol} и интервал {interval}")
                elif response.get('retCode') == 10002:  # Invalid request
                    logger.error(f"❌ Неверный запрос. Возможно символ {symbol} не поддерживается")
            
            return response
            
        except Exception as e:
            logger.error(f"❌ Исключение в get_klines: {e}", exc_info=True)
            return {
                'retCode': -1,
                'retMsg': str(e),
                'result': {'list': []}
