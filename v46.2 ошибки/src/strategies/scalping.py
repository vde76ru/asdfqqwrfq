...
class ScalpingStrategy(BaseStrategy):
    ...
    async def analyze(self, df: pd.DataFrame, symbol: str) -> TradingSignal:
        ...
        # Сначала получим цену, чтобы использовать её в случае ошибок
        current_price = df['close'].iloc[-1] if not df.empty else 0.0

        if not self.validate_dataframe(df):
            # ✅ ИСПРАВЛЕНО: Добавлены 'symbol' и 'price'
        ...
        try:
            ...
            if not market_conditions['suitable']:
                # ✅ ИСПРАВЛЕНО: Добавлены 'symbol' и 'price'
                return TradingSignal(symbol=symbol, action='WAIT', confidence=0, price=current_price, reason=market_conditions['reason'])
            ...
            # Принимаем решение
            return self._make_scalping_decision(scalping_signals, indicators, df, symbol)
    ...
    def _check_scalping_conditions(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(df) > 20:
                ...
                if len(ema_20) > 5 and ema_20.iloc[-5] > 0:
    ...
    def _calculate_scalping_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if TA_AVAILABLE:
                ...
                indicators['ema_fast_prev'] = float(ema_fast.iloc[-2])
                indicators['ema_slow_prev'] = float(ema_slow.iloc[-2])
                
                # RSI с коротким периодом
                rsi = RSIIndicator(df['close'], window=7).rsi()  # Короткий RSI для скальпинга
                ...
                indicators['ema_fast'] = ema_fast_series.iloc[-1]
                indicators['ema_slow'] = ema_slow_series.iloc[-1]
                indicators['ema_fast_prev'] = ema_fast_series.iloc[-2]
                indicators['ema_slow_prev'] = ema_slow_series.iloc[-2]
            ...
            indicators['momentum_3'] = (current_price - df['close'].iloc[-4]) / df['close'].iloc[-4] if len(df) > 4 else 0
            indicators['momentum_5'] = (current_price - df['close'].iloc[-6]) / df['close'].iloc[-6] if len(df) > 6 else 0
    ...
    def _make_scalping_decision(self, signals: Dict, indicators: Dict, df: pd.DataFrame, symbol: str) -> TradingSignal:
        """Принятие решения для скальпинга"""
        current_price = indicators.get('current_price', df['close'].iloc[-1] if not df.empty else 0.0)
        try:
            ...
            max_confidence = max([s['confidence'] for s in buy_signals + sell_signals], default=0)
            ...
            return TradingSignal(symbol=symbol, action='WAIT', confidence=0, price=current_price, reason=f'Ошибка решения: {e}')
    ...
    def _calculate_stochastic(self, df, k_period=5, d_period=3):
        ...
        k_val = float(k_percent.iloc[-1]) if pd.notna(k_percent.iloc[-1]) else 50.0
        d_val = float(d_percent.iloc[-1]) if pd.notna(d_percent.iloc[-1]) else 50.0
        return k_val, d_val
    ...
    def _calculate_atr(self, df, period=7):
        ...
        return float(atr.iloc[-1]) if pd.notna(atr.iloc[-1]) else 0.001
