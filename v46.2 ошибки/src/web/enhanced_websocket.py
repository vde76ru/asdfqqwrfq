...
class WebSocketBotIntegration:
    ...
    def _get_strategies_from_bot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.bot_manager, 'strategy_instances'):
                for name, strategy in self.bot_manager.strategy_instances.items():
                    strategies[name] = {
                        'active': True,
                        'trades_count': getattr(strategy, 'trades_count', 0),
                        'success_rate': getattr(strategy, 'success_rate', 0)
                    }
