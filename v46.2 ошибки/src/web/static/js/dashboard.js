...
class DashboardManager {
    ...
    async loadBotStatus() {
        try {
            ...
            // Используем правильные ID из dashboard.html
            ...
            if (!statusText) {
                console.error('Элемент bot-status-text не найден');
                return data;
            }
            ...
            if (data.is_running || data.running) {
                // Бот работает
                if (statusIndicator) {
                    statusIndicator.innerHTML = '<i class="fas fa-circle text-success"></i> <span id="bot-status-text">Работает</span>';
                }
                if (statusText) {
                ...
                // Бот остановлен
                if (statusIndicator) {
                    statusIndicator.innerHTML = '<i class="fas fa-circle text-danger"></i> <span id="bot-status-text">Остановлен</span>';
                }
                if (statusText) {
            ...
            if (data.cycles_count !== undefined) {
                const cyclesEl = document.getElementById('cycles-count');
                if (cyclesEl) cyclesEl.textContent = data.cycles_count;
            }
            
            if (data.active_pairs) {
                const pairsCount = document.getElementById('active-pairs-count');
                if (pairsCount) pairsCount.textContent = data.active_pairs.length;
                
                const pairsEl = document.getElementById('active-pairs');
                if (pairsEl) {
                    pairsEl.textContent = data.active_pairs.length > 0 
                        ? data.active_pairs.join(', ') 
                        : 'Нет активных пар';
                }
            }
            ...
            if (data.positions) {
                ...
                if (posCount) posCount.textContent = data.positions.count || 0;
            }
    ...
    initWebSocket() {
        ...
        socket.on('positions_update', (data) => {
            console.log('Получено обновление позиций:', data);
            this.updatePositions(data);
        });
    }
