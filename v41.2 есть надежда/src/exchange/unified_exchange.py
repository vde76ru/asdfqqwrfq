...
class CustomHTTPAdapter(HTTPAdapter):
    """Полностью исправленный HTTP адаптер"""
    
    def __init__(self, max_retries=None, **kwargs):
        """Упрощенная инициализация"""
        # Убираем ВСЕ проблемные параметры
        kwargs.pop('maxsize', None)
        kwargs.pop('pool_maxsize', None) 
        kwargs.pop('pool_connections', None)
    ...
    def init_poolmanager(self, *args, **kwargs):
        ...
        # Только безопасные параметры
        safe_params = ['cert_reqs', 'ca_cert_dir', 'ca_certs', 'ssl_version']
        for key, value in kwargs.items():
            if key in safe_params:
                safe_kwargs[key] = value
        
        # Добавляем только наш maxsize
        safe_kwargs['maxsize'] = 50
        
        try:
            return super().init_poolmanager(*args, **safe_kwargs)
        except Exception as e:
            # Последняя попытка - минимальные параметры
            return super().init_poolmanager(*args, maxsize=50)
...
class UnifiedExchangeClient(BaseExchangeClient):
    ...
    def _setup_connection_pool(self):
        """Полностью переписанная настройка пула"""
        try:
            import requests
            session = requests.Session()
            
            # Простая стратегия повторов
            ...
            # Создаем максимально простой адаптер
            ...
            logger.info("✅ Пул соединений настроен без предупреждений")
            
        except Exception as e:
            logger.warning(f"⚠️ Fallback к базовой сессии: {e}")
            try:
                import requests
