    ...
    def _init_signal_components(self):
        ...
        try:
            ...
            def safe_get_float(key: str, default: float) -> float:
                """Безопасно получает float значение из конфигурации"""
                try:
                    value = getattr(config, key, default)
                    if isinstance(value, str) and value.strip() == '':
                        logger.warning(f"Параметр '{key}' пуст, используется значение по умолчанию: {default}")
                        return default
                    return float(value)
                except (ValueError, TypeError):
                    logger.warning(f"Не удалось преобразовать '{key}' в float, используется значение по умолчанию: {default}")
                    return default
            ...
            self.order_book_analysis = OrderBookAnalysisStrategy(config=order_book_config)
        ...
        for symbol in self.active_pairs:
            try:
                ...
                if self.order_book_analysis:
                    try:
                        # >>> ИСПРАВЛЕНИЕ №3: Передаем exchange_client в стратегию <<<
                        signal = await self.order_book_analysis.analyze(symbol, exchange_client=self.exchange_client)
                        if signal:
                            strategies_data.append({
