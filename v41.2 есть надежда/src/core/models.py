...
class Signal(Base):
    ...
    def to_dict(self):
        ...
        # Используем getattr для безопасного доступа к полям, которых может не быть
        signal_type_val = getattr(self, 'signal_type', None)
        stop_loss_val = getattr(self, 'stop_loss', None)
        take_profit_val = getattr(self, 'take_profit', None)
        trade_id_val = getattr(self, 'trade_id', None)
        ...
        return {
            ...
            'symbol': self.symbol,
            'action': self.action,
            'signal_type': signal_type_val.value if signal_type_val else None,
