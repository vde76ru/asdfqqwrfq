...
class WhaleHuntingStrategy(BaseStrategy):
    ...
    async def analyze(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Фильтруем сигналы по указанному символу
            for signal in all_signals:
                if signal.get('symbol') == symbol:
                    ...
                    return {
                        ...
                        'price_target': None,  # WhaleHunting не предоставляет цель
                        'reason': signal.get('reason', '')
                    }
            
            # Если сигналов для данного символа нет
            return None
            
        except Exception as e:
            logger.error(f"❌ Ошибка анализа {symbol} в WhaleHuntingStrategy: {e}")
            return None
