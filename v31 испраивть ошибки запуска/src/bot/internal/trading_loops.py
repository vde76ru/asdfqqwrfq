...
async def _main_trading_loop(bot_instance):
    ...
    while not bot_instance._stop_event.is_set():
        try:
            ...
            if hasattr(bot_instance, '_analyze_market_conditions'):
                await bot_instance._analyze_market_conditions()
            
            # 2. Обновление позиций
            if hasattr(bot_instance, '_update_all_positions'):
    ...
    try:
        ...
        if hasattr(bot_instance, 'whale_hunting_strategy') and bot_instance.whale_hunting_strategy:
            ...
            task = asyncio.create_task(
                run_strategy_loop(bot_instance.whale_hunting_strategy, interval, "WhaleHunting")
            )
        ...
        if hasattr(bot_instance, 'sleeping_giants_strategy') and bot_instance.sleeping_giants_strategy:
            ...
            task = asyncio.create_task(
                run_strategy_loop(bot_instance.sleeping_giants_strategy, interval, "SleepingGiants")
            )
        ...
        if hasattr(bot_instance, 'order_book_analysis') and bot_instance.order_book_analysis:
            ...
            task = asyncio.create_task(
                run_strategy_loop(bot_instance.order_book_analysis, interval, "OrderBookAnalysis")
            )
...
async def run_strategy_loop(strategy, interval: int, name: str):
    """Универсальный цикл для запуска стратегии"""
    logger.info(f"🔄 Запуск цикла {name} с интервалом {interval}с")
    
    while True:
        try:
            logger.debug(f"🔄 {name}: начало цикла анализа")
            
            # Пытаемся найти подходящий метод для выполнения
            if hasattr(strategy, 'analyze'):
                signals = await strategy.analyze()
                if signals:
                    logger.info(f"🎯 {name}: сгенерировано {len(signals)} сигналов")
            elif hasattr(strategy, 'run'):
                await strategy.run()
            elif hasattr(strategy, 'start'):
                await strategy.start()
            else:
                logger.warning(f"⚠️ {name}: не найден подходящий метод запуска")
                break
            ...
            logger.error(f"❌ {name}: ошибка в цикле: {e}")
            await asyncio.sleep(interval * 2)  # Увеличенная пауза при ошибке
